...
// Neopixel
#define NUM_LEDS 103  // Number of LEDs
#define DATA_PIN 53   // Data-Pin
#define Rele1 29
#define Monedero 20

volatile bool flagMonedero = false;

int maxBright = 155;  // max brightness

byte playerBtnPin[] = { 43, 44 };  // Pins for buttons
byte playerLedPin[] = { 6, 9 };    // Pins for LEDs in buttons

int gameSpeedMin = 30;             // min game-speed
...
byte playerColor[] = { 160, 96 };  // red & green
int winRounds = 10;                // x winning rounds for winning game
int endZoneSize = 6;               // size endzone
int endZoneColor = 0;              // color endzone
//int displScore      =        0;

ShiftRegister74HC595<1> sr(50, 51, 52);
ShiftRegister74HC595<1> jr(47, 46, 45);
...
void setup() {
  ...
  attachInterrupt(3, monederoServicio, FALLING);
}
...
// *********************************
void loop() {
  //menu();le modifique esto porque el menu esta de mas no me sirve, asi que lo salte al juego directo
  Serial.print("El valor de flag monedero es");
  Serial.println(flagMonedero);
  game();
  if (flagMonedero == true) {  // voy a meter una interrupcion para ver el estado del monedero
